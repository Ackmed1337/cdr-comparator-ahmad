    ...
    <div className="mb-3">
      <div className="relative mb-3">
        ...
        <input
          ...
          className="w-full pl-10 pr-10 py-2 bg-slate-900 border border-slate-700 rounded-lg text-slate-300 placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200"
        />
        {(search || activeFilters.length > 0) && (
          <button
            onClick={handleClear}
            className="absolute right-3 top-2.5 text-slate-500 hover:text-slate-300 transition-colors duration-200"
          >
            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <button
            ...
            className={`px-3 py-1 rounded-full text-xs font-semibold transition-all duration-200 ${
              activeFilters.includes(filter.key)
                ...
                : 'bg-slate-800 border border-slate-700 text-slate-400 hover:border-blue-500 hover:text-blue-400'
            }`}
          >
            {filter.label}
          </button>
