...
const LendingRate = (props) => {
  ...
  return (
    <li className={`bg-slate-100/30 dark:bg-slate-800/30 p-2 rounded border-l-2 mb-1 ${compact ? 'border-border' : 'border-red-500'}`}>
      <div className={`text-base font-bold ${compact ? 'text-foreground' : 'text-red-600 dark:text-red-400'}`}>{(rate * 100).toFixed(2)}%</div>
